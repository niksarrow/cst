...
from src.model import check_mt_model_params, build_mt_model, build_mt_model_modified
...
def get_parser():
    ...
    if not parser.parse_known_args()[0].attention:
        ...
        if parser.parse_known_args()[0].transformer:
            ...
            parser.add_argument("--decoder_attention_heads_aux", type=int, default=8,
                                help="decoder_attention_heads_aux")
            ...
            parser.add_argument("--decoder_normalize_before_aux", type=bool_flag, default=False,
                                help="decoder_normalize_before_aux")
        else:
    ...
    # disfluent model parameters
    parser.add_argument("--n_dec_layers_aux", type=int, default=4,
                        help="Number of layers in the auxiliary decoders")
    parser.add_argument("--share_dec_aux", type=int, default=0,
                        help="Number of layers to share in the auxiliary decoders")
    parser.add_argument("--n_para_aux", type=int, default=0,
                       help="Number of parallel sentences in aux dataset (-1 for everything)")
    parser.add_argument("--para_dataset_aux", type=str, default="",
                        help="Parallel dataset aux (lang1-lang2:train12,valid12,test12;lang1-lang3:train13,valid13,test13)")
    parser.add_argument("--para_directions_aux", type=str, default="",
                        help="Training directions (lang1-lang2,lang2-lang1)")
    parser.add_argument("--reload_dec_aux", type=bool_flag, default=False,
                        help="Reload a pre-trained auxiliary decoder")
    parser.add_argument("--freeze_dec_emb_aux", type=bool_flag, default=False,
                        help="Freeze auxiliary decoder embeddings")
    parser.add_argument("--is_aux", type=bool_flag, default=False,
                        help="Is auxiliary decoder in use")
    parser.add_argument("--lambda_xe_para_aux", type=str, default="0",
                    help="Cross-entropy reconstruction coefficient (parallel auxiliary data)")
    parser.add_argument("--dec_optimizer_aux", type=str, default="enc_optimizer",
                        help="Auxiliary Decoder optimizer (SGD / RMSprop / Adam, etc.)")

    # parallel disfluent model (monolingual earlier)
    parser.add_argument("--n_mono_para", type=int, default=0,
                        help="Number of parallel sentences (-1 for everything)")
    parser.add_argument("--mono_para_directions", type=str, default="",
                                    help="Training directions (lang1,lang2)")
    parser.add_argument("--lambda_xe_mono_para", type=str, default="0",
                        help="Cross-entropy reconstruction coefficient (autoencoding)")
    parser.add_argument("--mono_para_dataset_lang1", type=str, default="",
                        help="Parallel dataset aux (lang1-lang2:train12,valid12,test12;lang1-lang3:train13,valid13,test13)")
    parser.add_argument("--mono_para_dataset_lang1_directions", type=str, default="",
                        help="Training directions (lang1-lang2,lang2-lang1)")
    parser.add_argument("--mono_para_dataset_lang2", type=str, default="",
                        help="Parallel dataset aux (lang1-lang2:train12,valid12,test12;lang1-lang3:train13,valid13,test13)")
    parser.add_argument("--mono_para_dataset_lang2_directions", type=str, default="",
                        help="Training directions (lang1-lang2,lang2-lang1)")
    parser.add_argument("--mono_para", type=bool_flag, default=False,
                                    help="Treating mono as para dataset")
...
def main(params):
    ...
    encoder, decoder_aux , decoder, discriminator, lm = build_mt_model_modified(params, data)
    ...
    trainer = TrainerMT(encoder, decoder, decoder_aux, discriminator, lm, data, params)
    ...
    for _ in range(trainer.epoch, params.max_epoch):
        ...
        while trainer.n_sentences < params.epoch_size:
            ...
            if params.lambda_xe_mono > 0:
                    trainer.enc_dec_step(lang, lang, params.lambda_xe_mono, mono=True)
            ...
            if params.lambda_xe_mono_para > 0:
                for lang1, lang2 in params.mono_para_directions: # (en,en), (es,es)
                        if(lang1 in  ['en']):
                            trainer.enc_dec_step(lang1, lang2, params.lambda_xe_mono_para, mono_para=True)
            ...
            if params.lambda_xe_para > 0:
                for lang1, lang2 in params.para_directions: # (en,es), (es,en)
                    #logger.info(" para : %s %s" % (lang1, lang2))
                        ...
                        trainer.enc_dec_step(lang1, lang2, params.lambda_xe_para, para=True)

            # MT training news-commentary data
            # E1->D2'->D2  and  E2->D1'->D1
            if params.lambda_xe_para_aux > 0:
               for lang1, lang2 in params.para_directions: # (en, es) , (es,en)
                   trainer.enc_dec_step(lang1, lang2, params.lambda_xe_para_aux, aux=True)
