...
def load_para_data_aux(params, data):
    """
    Load parallel auxiliary data.
    """
    assert len(params.para_dataset_aux) > 0

    for (lang1, lang2), paths in params.para_dataset_aux.items():

        assert lang1 in params.langs and lang2 in params.langs
        logger.info('============ Parallel auxiliary data (%s - %s)' % (lang1, lang2))
        ...
        for name, path in zip(['train', 'valid', 'test'], paths):
            #print('aux',name,path)
            if path == '':
                assert name == 'valid' or name == 'test'
                datasets.append((name, None))
                continue
            #assert name != 'train' or params.n_para_aux != 0
            ...
            #    assert data['dico'][lang2] == data2['dico']
            #print(data['dico'])
            # parallel auxiliary data
            para_data_aux = ParallelDataset(
                data1['sentences'], data1['positions'], data['dico'][lang1], params.lang2id[lang1],
                data2['sentences'], data2['positions'], data['dico'][lang2], params.lang2id[lang2],
                params
            )
            ...
            # remove too long sentences (train / valid only, test must remain unchanged)
            if name != 'test':
                para_data_aux.remove_long_sentences(params.max_len)

            # select a subset of sentences
            if name == 'train' and params.n_para_aux != -1:
                para_data.select_data(0, params.n_para_aux)
            # if name == 'valid':
            #     para_data.select_data(0, 100)
            # if name == 'test':
            #     para_data.select_data(0, 167)

            datasets.append((name, para_data_aux))

        assert (lang1, lang2) not in data['para_aux']
        data['para_aux'][(lang1, lang2)] = {k: v for k, v in datasets}

    logger.info('')

def load_mono_para_data(params, data):
    """
    Load monolingual parallel data.
    """
    assert len(params.mono_para_dataset_lang1) > 0

    # lang1 == lang2
    for (lang1, lang2), paths in params.mono_para_dataset_lang1.items():

        assert lang1 in params.langs and lang2 in params.langs
        logger.info('============ Mono Parallel data (%s.disfluent - %s)' % (lang1, lang1))
        ...
        for name, path in zip(['train', 'valid', 'test'], paths):
            ...
            # load data
            data1 = load_binarized(path.replace('X','.disfluent'), params)
            data2 = load_binarized(path.replace('X',''), params)
            # set_parameters(params, data1['dico'])
            # set_parameters(params, data2['dico'])
            ...
            # mono parallel data
            mono_para_data = ParallelDataset(
                data1['sentences'], data1['positions'], data['dico'][lang1], params.lang2id[lang1],
                data2['sentences'], data2['positions'], data['dico'][lang1], params.lang2id[lang1],
                params
            )

            # remove too long sentences (train / valid only, test must remain unchanged)
            if name != 'test':
                mono_para_data.remove_long_sentences(params.max_len)

            # select a subset of sentences
            if name == 'train' and params.n_mono_para != -1:
                mono_para_data.select_data(0, params.n_mono_para)
            # if name == 'valid':
            #     para_data.select_data(0, 100)
            # if name == 'test':
            #     para_data.select_data(0, 167)

            datasets.append((name, mono_para_data))

        assert (lang1, lang1) not in data['mono_para']
        data['mono_para'][(lang1, lang1)] = {k: v for k, v in datasets}

    logger.info('')

    # lang1 == lang2
    for (lang1, lang2), paths in params.mono_para_dataset_lang2.items():

        assert lang1 in params.langs and lang2 in params.langs
        logger.info('============ Mono Parallel data (%s.disfluent - %s)' % (lang2, lang2))
        ...
        for name, path in zip(['train', 'valid', 'test'], paths):
            ...
            # mono parallel data
            mono_para_data = ParallelDataset(
                data1['sentences'], data1['positions'], data['dico'][lang2], params.lang2id[lang2],
                data2['sentences'], data2['positions'], data['dico'][lang2], params.lang2id[lang2],
                params
            )
        ...
        assert (lang2, lang2) not in data['mono_para']
        data['mono_para'][(lang2, lang2)] = {k: v for k, v in datasets}
...
def check_all_data_params(params):
    ...
    # check parallel datasets mono_para lang1
    params.mono_para_dataset_lang1 = {k: v for k, v in [x.split(':') for x in params.mono_para_dataset_lang1.split(';') if len(x) > 0]}
    assert type(params.mono_para_dataset_lang1) is dict
    assert all(len(k.split('-')) == 2 for k in params.mono_para_dataset_lang1.keys())
    assert all(len(v.split(',')) == 3 for v in params.mono_para_dataset_lang1.values())
    params.mono_para_dataset_lang1 = {tuple(k.split('-')): tuple(v.split(',')) for k, v in params.mono_para_dataset_lang1.items()}
    assert not (params.n_mono_para == 0) ^ (all(v[0] == '' for v in params.mono_para_dataset_lang1.values()))
    
    # check parallel datasets mono_para lang2
    params.mono_para_dataset_lang2 = {k: v for k, v in [x.split(':') for x in params.mono_para_dataset_lang2.split(';') if len(x) > 0]}
    assert type(params.mono_para_dataset_lang2) is dict
    assert all(len(k.split('-')) == 2 for k in params.mono_para_dataset_lang2.keys())
    assert all(len(v.split(',')) == 3 for v in params.mono_para_dataset_lang2.values())
    params.mono_para_dataset_lang2 = {tuple(k.split('-')): tuple(v.split(',')) for k, v in params.mono_para_dataset_lang2.items()}
    assert not (params.n_mono_para == 0) ^ (all(v[0] == '' for v in params.mono_para_dataset_lang2.values()))

    # check parallel auxiliary datasets
    params.para_dataset_aux = {k: v for k, v in [x.split(':') for x in params.para_dataset_aux.split(';') if len(x) > 0]}
    assert type(params.para_dataset_aux) is dict
    assert all(len(k.split('-')) == 2 for k in params.para_dataset_aux.keys())
    assert all(len(v.split(',')) == 3 for v in params.para_dataset_aux.values())
    params.para_dataset_aux = {tuple(k.split('-')): tuple(v.split(',')) for k, v in params.para_dataset_aux.items()}
    assert not (params.n_para_aux == 0) ^ (all(v[0] == '' for v in params.para_dataset_aux.values()))
    for (lang1, lang2), (train_path, valid_path, test_path) in params.para_dataset_aux.items():
        assert lang1 < lang2 and lang1 in params.langs and lang2 in params.langs
        assert train_path == '' or os.path.isfile(train_path.replace('XX', lang1))
        assert train_path == '' or os.path.isfile(train_path.replace('XX', lang2))
    ...
    # check parallel aux directions
    params.para_directions_aux = [x.split('-') for x in params.para_directions_aux.split(',') if len(x) > 0]
    if len(params.para_directions_aux) > 0:
        assert params.n_para_aux != 0
        assert type(params.para_directions_aux) is list
        assert all(len(x) == 2 for x in params.para_directions_aux)
        params.para_directions_aux = [tuple(x) for x in params.para_directions_aux]
        assert len(params.para_directions_aux) == len(set(params.para_directions_aux))
        # check that every direction has an associated train set
        for lang1, lang2 in params.para_directions_aux:
            assert lang1 in params.langs and lang2 in params.langs
            k = (lang1, lang2) if lang1 < lang2 else (lang2, lang1)
            assert k in params.para_dataset_aux
            assert params.para_dataset_aux[k][0] != ''

    # check mono parallel directions
    params.mono_para_directions = [x.split('-') for x in params.mono_para_directions.split(',') if len(x) > 0]
    if len(params.mono_para_directions) > 0:
        assert params.n_mono_para != 0
        assert type(params.mono_para_directions) is list
        assert all(len(x) == 2 for x in params.mono_para_directions)
        params.mono_para_directions = [tuple(x) for x in params.mono_para_directions]
        assert len(params.mono_para_directions) == len(set(params.mono_para_directions))
        # check that every direction has an associated train set
        for lang1, lang2 in params.mono_para_directions:
            assert lang1 in params.langs and lang2 in params.langs
            k = (lang1, lang2) if lang1 < lang2 else (lang2, lang1)
            # assert k in params.mono_para_dataset_lang1
            # assert params.mono_para_dataset_lang1[k][0] != ''  #
        # k = ('es', 'es')
        # assert k in params.mono_para_dataset_lang2
        # print(params.mono_para_dataset_lang2[k][0])
    ...
    print(params.para_directions, len(params.para_directions))
    assert len(params.mono_directions) + len(params.para_directions) + len(params.pivo_directions) + len(params.mono_para_directions) > 0
    assert not params.n_mono == params.n_para == params.n_mono_para == 0
    ...
    assert not (params.lambda_xe_mono_para == "0") ^ (len(params.mono_para_directions) == 0)
    assert not (params.lambda_xe_para == "0") ^ (len(params.para_directions) == 0)
    assert not (params.lambda_xe_para_aux == "0") ^ (len(params.para_directions_aux) == 0)
    ...
    if len(params.mono_directions) == 0 and len(params.mono_para_directions) ==0:
...
def load_data(params, mono_only=False, mono_para=False):
    """
    ...
    """
    data = {'dico': {}, 'mono': {}, 'para': {}, 'back': {}, 'para_aux': {}, 'mono_para': {}}
    ...
    # parallel aux datasets
    if params.is_aux:
        load_para_data_aux(params, data)
    # monolingual datasets
    if not mono_para:
        load_mono_data(params, data)
    if mono_para:
        load_mono_para_data(params, data)
        #load_mono_data(params,data)
    ...
    #print("data['dico']",data['dico'])
    if data['dico']['en'] == data['dico']['es'] :
        print('Dicos are equal')
    else:
        print('Dicos are different')

    #input('nikhil')
    print(data['dico']['es'],data['dico']['en'])
    print("data['mono']",data['mono'])
    print("data['para']", data['para'])
    print("data['back']", data['back'])
    print("data['para_aux']", data['para_aux'])
    print("data['mono_para']", data['mono_para'])
    # data summary
