...
class EvaluatorMT(object):
    ...
    def __init__(self, trainer, data, params):
        ...
        self.decoder_aux = trainer.decoder_aux
    ...
    def get_pair_for_mono(self, lang): # check if something is to be done here.
        """
    ...
    def eval_para(self, lang1, lang2, data_type, scores, src_lang):
        """
        ...
        assert data_type in ['valid', 'test']
        self.encoder.eval()
        #self.decoder_aux.eval() # will notify all layers that you are in eval mode,that way,
        ...
        lang1_id = params.lang2id[lang1]
        lang2_id = params.lang2id[lang2]
        #print(lang1, lang1_id, lang2, lang2_id)
        txt = []
        hypothesis = []
        # for perplexity
        loss_fn2 = nn.CrossEntropyLoss(weight=self.decoder.loss_fn[lang2_id].weight, size_average=False)
        #loss_fn2_aux = nn.CrossEntropyLoss(weight=self.decoder_aux.loss_fn[lang2_id].weight, size_average=False)
        ...
        for batch in self.get_iterator(data_type, lang1, lang2):
            ...
            src_lengths = len2 # len1
            # encode / decode / generate
            encoded = self.encoder(sent1, len1, lang1_id)
            decoded = self.decoder(encoded, sent2[:-1], lang2_id)
            #decoded_aux, s2_, l2_, _ = self.decoder_aux.generate(encoded, lang2_id)
            #decoded_aux_teacher = self.decoder_aux(encoded, sent2[:-1], lang2_id)
            #decoded = self.decoder(decoded_aux, sent2[:-1], lang2_id) # change here too
            ...
            #xe_loss_aux += loss_fn2_aux(decoded_aux_teacher.x.view(-1, n_words2), sent2[1:].view(-1)).item()
            #count_aux += (len2-1).sum().item()

            # convert to text
            #hypothesis.extend(convert_to_text(s2_, l2_, self.dico[lang2], lang2_id, self.params))
        ...
        #with open(hyp_path+'.aux', 'w', encoding='utf-8') as f:
        #    f.write('\n'.join(hypothesis) + '\n')
        #restore_segmentation(hyp_path+'.aux')
        ...
        #bleu_aux = eval_moses_bleu(ref_path, hyp_path+'.aux')
        #logger.info("BLEU aux %s %s : %f" % (hyp_path+'.aux', ref_path, bleu_aux))
        #logger.info("perplexity :%f", np.exp(xe_loss_aux/count_aux))
        # update scores
        scores['ppl_%s_%s_%s' % (lang1, lang2, data_type)] = np.exp(xe_loss / count)
        scores['bleu_%s_%s_%s' % (lang1, lang2, data_type)] = bleu

    # Here we evaluate at the end of decoder2, i.e. Enc1->Aux_Dec2->Dec2
    # Not needed, as we don't require any evauation at the end of Decoder aux.
    def eval_para_aux(self, lang1, lang2, data_type, scores):
        """
        Evaluate lang1 -> lang2 perplexity and BLEU scores.
        """
        logger.info("Evaluating auxiliary %s -> %s (%s) ..." % (lang1, lang2, data_type))
        assert data_type in ['valid', 'test']
        self.encoder.eval()
        self.decoder_aux.eval()
        self.decoder.eval() # will notify all layers that you are in eval mode,that way,
                            # batchnorm or dropout layers will work in eval mode instead of training mode.
        ...
        # for perplexity
        loss_fn2 = nn.CrossEntropyLoss(weight=self.decoder_aux.loss_fn[lang2_id].weight, size_average=False)
        n_words2 = self.params.n_words[lang2_id]
        count = 0
        xe_loss = 0
        ...
        for batch in self.get_iterator(data_type, lang1, lang2):
            ...
            # encode / decode / decode / generate
            encoded = self.encoder(sent1, len1, lang1_id)
            decoded = self.decoder_aux(encoded, sent2[:-1], lang2_id, src_lengths=len2)
            sent2_, len2_, _ = self.decoder.generate(encoded, lang2_id)
    ...
    def run_all_evals(self, epoch):
        ...
        with torch.no_grad():
            ...
            # we want to evaluate only on valid/test fisher dataset, which is present in para.
            for lang1, lang2 in self.data['para'].keys():
                # logger.info('There are the langs for evaluation : %s, %s' % (lang1, lang2))
                # logger.info('Evaluatiing in this direction %s, %s' % (lang1, lang2))
                for data_type in ['valid', 'test']:
                    self.eval_para(lang1, lang2, data_type, scores, src_lang=lang1)
                    self.eval_para(lang2, lang1, data_type, scores, src_lang=lang2)
