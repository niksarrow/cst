# Adapted from facebookresearch/UnsupervisedMT/NMT
# Nikhil Saini
...
class TrainerMT(MultiprocessingEventLoop):
    ...
    def __init__(self, encoder, decoder, decoder_aux, discriminator, lm, data, params): # decoder 1, decoder 2
        """
        Initialize trainer.
        """
        ...
        if params.dec_optimizer_aux == 'enc_optimizer':
            params.dec_optimizer_aux = params.enc_optimizer
        self.enc_optimizer = get_optimizer(enc_params, params.enc_optimizer) if len(enc_params) > 0 else None
        self.dec_optimizer_aux = get_optimizer(decoder_aux.parameters(), params.dec_optimizer_aux)
        ...
        self.model_opt = {
            ...
            'dec_aux': (self.decoder_aux, self.dec_optimizer_aux),
        ...
        self.freeze_dec_emb = self.params.freeze_dec_emb_aux
        ...
        for lang in params.mono_directions:
            self.stats['xe_costs_mono_%s_%s' % (lang, lang)] = []
        for lang1, lang2 in params.mono_para_directions:
            self.stats['xe_costs_mono_para_%s_%s' % (lang1, lang2)] = []
        for lang1, lang2 in params.para_directions:
            self.stats['xe_costs_para_%s_%s' % (lang1, lang2)] = []
        for lang1, lang2 in params.para_directions_aux:
            self.stats['xe_costs_aux_%s_%s' % (lang1, lang2)] = []   # change xe_costs_%s_%s lang1,lang2
        ...
        for lang in params.langs:
            ...
            self.stats['lmd_costs_aux_%s' % lang] = []
        ...
        parse_lambda_config(params, 'lambda_xe_mono_para')
        parse_lambda_config(params, 'lambda_xe_para_aux')
    ...
    def get_iterator(self, iter_name, lang1, lang2, back, aux, mono_para=False):
        """
        ...
        if lang2 is None and mono_para == False:
            dataset = self.data['mono'][lang1]['train']
        elif back:
            dataset = self.data['back'][(lang1, lang2)]
        elif aux:
            k = (lang1, lang2) if lang1 < lang2 else (lang2, lang1)
            dataset = self.data['para_aux'][k]['train']
        elif mono_para == True:
            dataset = self.data['mono_para'][(lang1, lang1)]['train']
        else:
            k = (lang1, lang2) if lang1 < lang2 else (lang2, lang1)
            dataset = self.data['para'][k]['train']
        iterator = dataset.get_iterator(shuffle=False, group_by_size=False)()
        self.iterators[key] = iterator
        return iterator

    def get_batch(self, iter_name, lang1, lang2, back=False, aux=False, mono_para=False):
        """
        ...
        if iterator is None:
            iterator = self.get_iterator(iter_name, lang1, lang2, back, aux, mono_para)
        try:
            batch = next(iterator)
        except StopIteration:
            iterator = self.get_iterator(iter_name, lang1, lang2, back, aux, mono_para)
            batch = next(iterator)
        return batch if (lang2 is None or lang1 < lang2 or back or aux or mono_para) else batch[::-1]
    ...
    def add_noise(self, words, lengths, lang_id):
        ...
        # words, lengths = self.synonym_insertion(words, lengths, lang_id)
        # words, lengths = self.filler_insertion(words, lengths, lang_id)
        # words, lengths = self.pronoun_repetition(words, lengths, lang_id)
        # words, lengths = self.phrase_repetition(words, lengths, lang_id)
    ...
    def enc_dec_step(self, lang1, lang2, lambda_xe, back=False, aux=False, para=False, mono=False, mono_para=False):
        """
        ...
        """
        # logger.info("Traning : %s %s" % (lang1, lang2))
        params = self.params
        assert lang1 in params.langs and lang2 in params.langs
        lang1_id = params.lang2id[lang1]
        lang2_id = params.lang2id[lang2]
        if aux or mono or mono_para:
            loss_fn = self.decoder.loss_fn[lang2_id]
        elif para:
            loss_fn = self.decoder_aux.loss_fn[lang2_id]
            #loss_fn = self.decoder.loss_fn[lang2_id]
        n_words = params.n_words[lang2_id]
        self.encoder.train()

        if aux:
            self.decoder_aux.train()
            self.decoder.train()
        elif mono or mono_para:
            self.decoder.train()
        elif para:
            self.decoder_aux.train()
        ...
        elif lang1 == lang2 and mono_para == False:
            sent1, len1 = self.get_batch('encdec', lang1, None)
            sent2, len2 = sent1, len1
        elif aux:
            (sent1, len1), (sent2, len2) = self.get_batch('encdec_aux', lang1, lang2, aux=True)
        elif mono_para:
            (sent1, len1), (sent2, len2) = self.get_batch('encdec_mono_para', lang1, lang1, mono_para=True)
        else:
        ...
        if aux:
            decoded_aux = self.decoder_aux(encoded, sent2[:-1], lang2_id, src_lengths=len2)

        # cross-entropy scores / loss
        if aux:
            scores = self.decoder(decoded_aux, sent2[:-1], lang2_id)
        elif para:
            #print('para:trainer:len2',len2)
            scores = self.decoder_aux(encoded, sent2[:-1], lang2_id, src_lengths=len1) # scores is latent state here
            #scores = self.decoder(encoded, sent2[:-1], lang2_id)
            # print("Decoder forward complete")
        elif mono:
            scores = self.decoder(encoded, sent2[:-1], lang2_id)
        elif mono_para:
            scores = self.decoder(encoded, sent2[:-1], lang2_id)
        ...
        if para:
            xe_loss = loss_fn(scores.x.view(-1, n_words), sent2[1:].view(-1))
        ...
        if back:
            ...
            self.stats['xe_costs_aux_%s_%s' % (lang1, lang2)].append(xe_loss.item())
        elif para:
            self.stats['xe_costs_para_%s_%s' % (lang1, lang2)].append(xe_loss.item())
        elif mono:
            self.stats['xe_costs_mono_%s_%s' % (lang1, lang2)].append(xe_loss.item())
        elif mono_para:
            self.stats['xe_costs_mono_para_%s_%s' % (lang1, lang2)].append(xe_loss.item())
        ...
        # optimizer
        if aux:
            self.zero_grad(['enc', 'dec_aux', 'dec'])
            loss.backward()
            self.update_params(['enc', 'dec_aux', 'dec'])
        elif para:
            self.zero_grad(['enc', 'dec_aux'])
            #self.zero_grad(['enc', 'dec'])
            loss.backward()
            self.update_params(['enc', 'dec_aux'])
            #self.zero_grad(['enc', 'dec'])
        elif mono:
            self.zero_grad(['enc', 'dec'])
            loss.backward()
    ...
    def iter(self):
        ...
        self.n_iter += 1
        self.n_total_iter += 1
        n_batches = len(self.params.mono_directions) + len(self.params.para_directions) + len(self.params.para_directions_aux) + len(self.params.back_directions) + len(self.params.pivo_directions) + len(self.params.mono_para_directions)
    ...
    def print_stats(self):
        ...
        if self.n_iter % 50 == 0:
            ...
            for lang in self.params.mono_directions:
                mean_loss.append(('XE-%s-%s' % (lang, lang), 'xe_costs_mono_%s_%s' % (lang, lang)))
            for lang1, lang2 in self.params.mono_para_directions:
                mean_loss.append(('XE-%s-%s' % (lang1, lang2), 'xe_costs_mono_para_%s_%s' % (lang1, lang2)))                            
            for lang1, lang2 in self.params.para_directions:
                mean_loss.append(('XE-%s-%s' % (lang1, lang2), 'xe_costs_para_%s_%s' % (lang1, lang2)))
            for lang1, lang2 in self.params.para_directions_aux:
                mean_loss.append(('XE-%s-%s' % (lang1, lang2), 'xe_costs_aux_%s_%s' % (lang1, lang2)))
    ...
    def test_sharing(self):
        ...
        """
        test_sharing(self.encoder, self.decoder, self.decoder_aux, self.lm, self.params)
