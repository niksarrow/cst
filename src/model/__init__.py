...
LatentState = namedtuple('LatentState', 'dec_input, dis_input, input_len, x') # using namedtuple you can access using key value, index, or getattr.
...
def check_mt_model_params(params):
    ...
    assert 0 <= params.share_dec_aux <= params.n_dec_layers_aux # aux
    ...
    if params.lm_before == params.lm_after == 0:
        ...
        assert params.lm_share_emb is False or not (params.freeze_enc_emb or params.freeze_dec_emb or params.freeze_dec_emb_aux)
    ...
    if params.pretrained_emb == '':
        ...
        assert not params.freeze_dec_emb_aux
        ...
        if params.share_encdec_emb:
            assert params.freeze_enc_emb == params.freeze_dec_emb == params.freeze_dec_emb_aux
        else:
            assert not (params.freeze_enc_emb and params.freeze_dec_emb and params.freeze_dec_emb_aux)
...
# not needed
def build_mt_model_modified(params, data, cuda=True):
    """
    Build machine translation model.
    """
    if params.attention:
        from .attention import build_attention_model_modified
        return build_attention_model_modified(params, data, cuda=cuda)
    else:
        from .seq2seq import build_seq2seq_model_modified
        return build_seq2seq_model_modified(params, data, cuda=cuda)
