...
from .pretrain_embeddings import initialize_embeddings, initialize_embeddings_modified
...
def build_transformer_enc_dec_modified(params):
    from .transformer import TransformerEncoder, TransformerDecoder, TransformerDecoder_aux
    ...
    encoder = TransformerEncoder(params)
    logger.info("")
    logger.info("============ Building transformer attention model - Aux Decoder ...")
    decoder_aux = TransformerDecoder_aux(params, encoder)
    logger.info("")
    logger.info("============ Building transformer attention model - Decoder ...")
    decoder = TransformerDecoder(params, encoder, decoder_aux)
    logger.info("")
    return encoder, decoder_aux, decoder
...
def build_attention_model_modified(params, data, cuda=True):
    """
    Build a encoder / auxiliary decoder / decoder , and the decoder reconstruction loss function and auxiliary decoder loss function.
    """
    # encoder / decoder / discriminator
    if params.transformer:
        encoder, decoder_aux, decoder = build_transformer_enc_dec_modified(params)
    else:
        encoder, decoder = build_lstm_enc_dec(params)
    if params.lambda_dis not in ["0", "-1"]:
        logger.info("============ Building attention model - Discriminator ...")
    ...
    # loss function for auxiliary decoder reconstruction
    loss_fn = []
    for n_words in params.n_words:
        loss_weight = torch.FloatTensor(n_words).fill_(1)
        loss_weight[params.pad_index] = 0
    ...
    decoder_aux.loss_fn = nn.ModuleList(loss_fn)
    ...
    if cuda:
        ...
        if len(params.vocab) > 0:
            decoder.vocab_mask_neg = [x.cuda() for x in decoder.vocab_mask_neg]
            decoder_aux.vocab_mask_neg = [x.cuda() for x in decoder.vocab_mask_neg]
        if discriminator is not None:
            discriminator.cuda()
        if lm is not None:
            lm.cuda()
        ...
        assert not (getattr(params, 'cpu_thread', False)) ^ (data is None)
        if data is not None:
            initialize_embeddings_modified(encoder, decoder_aux, decoder, params, data)

        # reload encoder / decoder_aux / decoder / discriminator
        if params.reload_model != '':
            assert os.path.isfile(params.reload_model)
            logger.info("Reloading model from %s ..." % params.reload_model)
            reloaded = torch.load(params.reload_model)
            if params.reload_enc:
                ...
                enc = reloaded.get('enc', reloaded.get('encoder'))
                reload_model(encoder, enc, encoder.ENC_ATTR)
            if params.reload_dec_aux:
                logger.info("Reloading auxiliary decoder...")
                dec = reloaded.get('dec_aux', reloaded.get('decoder_aux'))
                reload_model(decoder_aux, dec_aux, decoder_aux.DEC_ATTR)
            if params.reload_dec:
                logger.info("Reloading decoder...")
                dec = reloaded.get('dec', reloaded.get('decoder'))
                reload_model(decoder, dec, decoder.DEC_ATTR)
    ...
    # log models
    enc_decaux_dec_params = set(
        p
        for module in [encoder, decoder_aux, decoder]
        for p in module.parameters()
        if p.requires_grad
    )
    num_encdecauxdec_params = sum(p.numel() for p in enc_decaux_dec_params)
    logger.info("============ Model summary")
    logger.info("Number of enc+dec_aux+dec parameters: {}".format(num_encdecauxdec_params))
    logger.info("Encoder: {}".format(encoder))
    logger.info("Auxiliary Decoder: {}".format(decoder_aux))
    logger.info("Decoder: {}".format(decoder))
    logger.info("Discriminator: {}".format(discriminator))
    logger.info("LM: {}".format(lm))
    logger.info("")

    return encoder, decoder_aux, decoder, discriminator, lm
