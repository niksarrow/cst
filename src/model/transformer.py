...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src_tokens, src_lengths, lang_id):
        ...
        x += self.embed_positions(src_tokens)  # adding positional embeddins to token_embeddings
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, args, encoder, decoder):
        ...
        self.decoder_class = decoder.__class__
        ...
        # print('BOS')
        # for i in self.bos_index:
        #     print(i)
        # print('PAD', 'EOS')
        # print(self.pad_index, self.eos_index)
        # print('N_WORDS')
        # for i in self.n_words:
        #     print(i)
        # input('Decoder')
    ...
    def forward(self, encoded, y, lang_id, one_hot=False, incremental_state=None):
        ...
        encoder_out = encoded.dec_input # encoded could be from encoder or decoder_aux
        embed_tokens = self.embeddings[lang_id]
    ...
    def generate(self, encoded, lang_id, max_len=200, sample=False, temperature=None):
        ...
        while cur_len < max_len:
            ...
            scores = self.forward(encoded, decoded[:cur_len], lang_id, one_hot, incremental_state=None)
...
class TransformerDecoder_aux(nn.Module):
    """Transformer decoder."""

    DEC_ATTR = ['n_langs', 'n_words', ('share_lang_emb', False), ('share_encdec_emb', False), ('share_decpro_emb', False), ('share_dec_aux', False), 'dropout', 'eos_index', 'pad_index', 'bos_index']
    ...
    def __init__(self, args, encoder):
        ...
        self.share_decpro_emb = args.share_decpro_emb
        self.share_output_emb = args.share_output_emb
        self.share_dec_aux = args.share_dec_aux
        self.freeze_dec_emb_aux = args.freeze_dec_emb_aux
        self.encoder_class = encoder.__class__
        self.beam_size = args.beam_size
        self.length_penalty = args.length_penalty
        self.padding_idx = args.pad_index
        ...
        self.layers = nn.ModuleList()
        for k in range(args.decoder_layers): # check here
            # share bottom share_dec layers
            layer_is_shared = (k < args.share_dec) # check here
            if layer_is_shared:
                logger.info("Sharing decoder transformer parameters for layer %i" % k)
    ...
    def forward(self, encoded, y, lang_id, one_hot=False, incremental_state=None, src_lengths=None):
        assert not one_hot, 'one_hot=True has not been implemented for transformer'
        assert type(lang_id) is int
        ...
        if incremental_state is not None:
            prev_output_tokens = prev_output_tokens[-1:, :]  # only keep last time step
        x = self.embed_scale * embed_tokens(prev_output_tokens)
        x = x.detach() if self.freeze_dec_emb_aux else x
        x += positions
        x = F.dropout(x, p=self.dropout, training=self.training)
        # decoder layers
        for layer in self.layers:
        ...
        src_lengths = torch.zeros(y.shape[1])
        src_lengths[0] = y.shape[0]

        # we calculate new padding mask
        return LatentState(
            input_len= src_lengths,
            dec_input={
                'encoder_out': state_x,  # T x B x C
                'encoder_padding_mask':  y.t().eq(self.padding_idx),
            },
            dis_input=state_x,
            x=x,
        )
    ...
    def generate(self, encoded, lang_id, max_len=200, sample=False, temperature=None):
        ...
        latent_state = LatentState(input_len=prev_lengths, dec_input={'encoder_out':latent_state.dec_input['encoder_out'],
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, args):
        super().__init__()
        self.embed_dim = args.encoder_embed_dim  # each sentence is a vector of this dim (say 300)
        self.self_attn = MultiheadAttention(     # self attention layer in encoder
    ...
    def forward(self, x, encoder_padding_mask): # x is B * emb_dim (B is batch size) this is according to my understanding of paper.(implementation might be little different)
        residual = x
        x = self.maybe_layer_norm(0, x, before=True)
        x, _ = self.self_attn(query=x, key=x, value=x, key_padding_mask=encoder_padding_mask) # encoder padding mask ensures that padded words in a sentence are not given any attention
        x = F.dropout(x, p=self.dropout, training=self.training)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.self_attn = MultiheadAttention(    # self attention in decoder
        ...
        self.relu_dropout = args.relu_dropout
        self.normalize_before = args.encoder_normalize_before
        self.encoder_attn = MultiheadAttention(  # self attn layer which attends encoder output (K,V pairs)
            self.embed_dim, args.decoder_attention_heads,
            dropout=args.attention_dropout,
        )
    ...
    def forward(self, x, encoder_out, encoder_padding_mask, incremental_state=None): # encoder_out contains contains (K,V) pairs of encoder output, encoder pad mask is as usual.
        residual = x # x is target sentence which is to be predicted.
        x = self.maybe_layer_norm(0, x, before=True)
        x, _ = self.self_attn(
