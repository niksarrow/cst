...
def test_sharing(encoder, decoder, decoder_aux, lm, params):
    """
    ...
    if params.freeze_dec_emb_aux:
        for i in range(params.n_langs):
            k = 'dec_emb_aux_%i' % i
            if k in hashs:
                assert hash_data(decoder_aux.embeddings[i].weight) == hashs[k]
            else:
                hashs[k] = hash_data(decoder_aux.embeddings[i].weight)
    ...
    if params.share_encdec_emb:
        ...
        for i in range(1, params.n_langs):
            assert_equal(decoder_aux.embeddings[i].weight, decoder_aux.embeddings[0].weight)
    # LSTM layers
    if not params.transformer:
        ...
        for k in range(params.n_dec_layers_aux):
            if k + 1 <= params.share_dec_aux:
                for i in range(1, params.n_langs):
                    for name in LSTM_PARAMS:
                        if k == 0:
                            assert_equal(getattr(decoder_aux.lstm1[i], name % k), getattr(decoder_aux.lstm1[0], name % k))
                        else:
                            assert_equal(getattr(decoder_aux.lstm2[i], name % (k - 1)), getattr(decoder_aux.lstm2[0], name % (k - 1)))
    # attention layers : remaining here
    ...
    if params.lstm_proj:
        if params.share_lstm_proj:
            for i in range(1, params.n_langs):
                ...
                assert_equal(decoder_aux.lstm_proj_layers[i].weight, decoder_aux.lstm_proj_layers[0].weight)
                assert_equal(decoder_aux.lstm_proj_layers[i].bias, decoder_aux.lstm_proj_layers[0].bias)
    # projection layers
    if params.share_decpro_emb:
        for i in range(params.n_langs):
            assert_equal(decoder.proj[i].weight, decoder.embeddings[i].weight)
            assert_equal(decoder_aux.proj[i].weight, decoder_aux.embeddings[i].weight)
        if params.share_lang_emb:
            assert params.share_output_emb
            for i in range(1, params.n_langs):
                assert_equal(decoder.proj[i].bias, decoder.proj[0].bias)
                assert_equal(decoder_aux.proj[i].bias, decoder_aux.proj[0].bias)
    elif params.share_output_emb:
        assert params.share_lang_emb
        for i in range(1, params.n_langs):
            assert_equal(decoder.proj[i].weight, decoder.proj[0].weight)
            assert_equal(decoder.proj[i].bias, decoder.proj[0].bias)
            assert_equal(decoder_aux.proj[i].weight, decoder_aux.proj[0].weight)
            assert_equal(decoder_aux.proj[i].bias, decoder_aux.proj[0].bias)
    ...
    if lm is not None:
        ...
        # decoder : check if something is remaining here.
        if lm.use_lm_dec:
            # embedding layers
            if params.lm_share_emb:
                for i in range(params.n_langs):
